...
// Modern Da Vinci Luxury Edition
// Evolved from Block A & B with laboratory aesthetic layered in

// ============================================================================
// COLOR PALETTE: Evolution from Block A + Laboratory Accents
// ============================================================================
#let cream-bg = rgb("#E8E4DC")        // Block A foundation
#let rich-black = rgb("#1A1A1A")      // Primary text
#let deep-slate = rgb("#2F4F4F")      // Technical annotations
#let bright-cyan = rgb("#00CED1")     // Breakthrough highlights
#let warm-amber = rgb("#FFB347")      // Insights & key moments
#let soft-gray = rgb("#8B8B8B")       // Marginalia

// ============================================================================
// PAGE SETUP: 6x9 Luxury Book
// ============================================================================
#set page(
  width: 6in,
  height: 9in,
  ...
  footer: context {
    ...
    if page-num > 2 [
      #line(length: 100%, stroke: 0.3pt + soft-gray.lighten(30%))
      #v(-4pt)
      #set text(9.5pt, fill: rich-black, font: "Georgia")
...
// ============================================================================
// TYPOGRAPHY: Sophisticated & Readable
// ============================================================================
#set text(
  font: "Georgia",
  size: 11.5pt,
...
#let geometry-divider() = {
  align(center)[
    #v(0.25in)
    #box(width: 2in, height: 0.6in)[
      #place(
        center + horizon,
        {
          // Central circle
          circle(radius: 8pt, stroke: 0.5pt + deep-slate)
          // Concentric circles
          circle(radius: 12pt, stroke: 0.3pt + bright-cyan.lighten(30%))
          circle(radius: 16pt, stroke: 0.3pt + deep-slate.lighten(50%))

          // Horizontal lines
          line(start: (-40pt, 0pt), end: (-20pt, 0pt), stroke: 0.5pt + deep-slate)
          line(start: (20pt, 0pt), end: (40pt, 0pt), stroke: 0.5pt + deep-slate)

          // Small accent dots
          place(dx: -45pt, circle(radius: 1.5pt, fill: bright-cyan))
          place(dx: 45pt, circle(radius: 1.5pt, fill: bright-cyan))
        }
      )
    ]
...
// Laboratory annotation box
#let lab-note(content) = {
  block(
    width: 100%,
    inset: (left: 0.4in, right: 0.4in, top: 0.2in, bottom: 0.2in),
    fill: deep-slate.lighten(92%),
    stroke: (left: 2pt + deep-slate),
    radius: 2pt,
    {
      corner-accent(color: deep-slate)
      set text(size: 10pt, fill: deep-slate.darken(10%), style: "italic")
...
// ============================================================================
// HEADING STYLES: Evolved & Architectural
// ============================================================================
...
#show heading.where(level: 1): it => {
  ...
  block(
    width: 100%,
    {
      ...
      align(center)[
        #box(width: 3.5in, height: 40pt)[
          #place(
            center + horizon,
            {
              // Top line with breaks
              line(start: (-60pt, -15pt), end: (-20pt, -15pt), stroke: 1pt + deep-slate)
              line(start: (20pt, -15pt), end: (60pt, -15pt), stroke: 1pt + deep-slate)

              // Center diamond
              polygon(
                fill: bright-cyan,
                stroke: 0.5pt + deep-slate,
                (0pt, -8pt),
                (8pt, 0pt),
                (0pt, 8pt),
                (-8pt, 0pt),
              ...
              line(start: (-70pt, -15pt), end: (-65pt, -15pt), stroke: 0.5pt + warm-amber)
              line(start: (65pt, -15pt), end: (70pt, -15pt), stroke: 0.5pt + warm-amber)
            }
          )
        ]
        ...
        // Bottom accent line
        #box(width: 2in, height: 3pt)[
          #place(
            center,
            line(length: 2in, stroke: 1.5pt + gradient.linear(bright-cyan, deep-slate, bright-cyan))
          )
        ]
      ]
    }
...
#show heading.where(level: 2): it => {
  ...
  block(
    width: 100%,
    {
      // Side technical bracket
      grid(
        columns: (auto, 1fr),
        column-gutter: 15pt,

        // Left bracket design
        box(width: 8pt, height: 100%)[
          #place(
            top + left,
            {
              // Vertical line
              line(start: (4pt, 0pt), end: (4pt, 40pt), stroke: 2pt + deep-slate)
              // Top horizontal
              line(start: (4pt, 0pt), end: (0pt, 0pt), stroke: 0.5pt + bright-cyan)
              // Bottom horizontal
              line(start: (4pt, 40pt), end: (0pt, 40pt), stroke: 0.5pt + bright-cyan)
            }
          )
        ],
...
#show heading.where(level: 3): it => {
  ...
  // Subheading with cyan underline
  stack(
    dir: ttb,
    spacing: 8pt,
    text(
    ...
    line(length: 1.2in, stroke: 1pt + bright-cyan.lighten(20%))
  )
...
// ============================================================================
// SPECIAL ELEMENTS: Laboratory Aesthetic
// ============================================================================

// Pull Quote - Cyan highlight (breakthrough moments)
#let pull-quote(content) = {
  v(0.35in)
  block(
    width: 100%,
    inset: (left: 0.6in, right: 0.6in, top: 0.25in, bottom: 0.25in),
    fill: bright-cyan.lighten(90%),
    stroke: (
      left: 3pt + bright-cyan,
      top: 0.5pt + bright-cyan.lighten(50%),
      bottom: 0.5pt + bright-cyan.lighten(50%)
    ),
    {
      corner-accent(size: 15pt, color: bright-cyan)
      set text(size: 13pt, style: "italic", fill: deep-slate, weight: "semibold")
...
// Insight Box - Amber highlight (key discoveries)
#let insight-box(title, content) = {
  v(0.3in)
  block(
    width: 100%,
    inset: 0.3in,
    radius: 3pt,
    fill: warm-amber.lighten(90%),
    stroke: (top: 2pt + warm-amber, bottom: 2pt + warm-amber),
    {
      set par(first-line-indent: 0em, leading: 0.65em)
      ...
      text(
        size: 11pt,
        weight: "bold",
        fill: warm-amber.darken(30%),
        smallcaps(title)
      )

      v(0.12in)

      text(size: 10.5pt, fill: rich-black, content)
    }
  )
  v(0.3in)
}
...
#let drop-cap(letter, text-content) = {
  grid(
    columns: (auto, 1fr),
    column-gutter: 8pt,
...
#page(
  ...
  #align(center)[
    ...
    #box(width: 4in, height: 60pt)[
      #place(
        center + horizon,
        {
          // Outer brackets
          line(start: (-80pt, -20pt), end: (-80pt, 20pt), stroke: 1pt + deep-slate)
          line(start: (-80pt, -20pt), end: (-70pt, -20pt), stroke: 1pt + deep-slate)
          line(start: (-80pt, 20pt), end: (-70pt, 20pt), stroke: 1pt + deep-slate)
          ...
          // Center design
          circle(radius: 6pt, stroke: 1pt + bright-cyan)
          circle(radius: 10pt, stroke: 0.5pt + deep-slate.lighten(40%))
          circle(radius: 14pt, stroke: 0.3pt + bright-cyan.lighten(50%))

          // Technical marks
          for i in range(8) {
            let angle = i * 45deg
            let x = 18pt * calc.cos(angle)
            let y = 18pt * calc.sin(angle)
            place(dx: x, dy: y, circle(radius: 1pt, fill: warm-amber))
          }
        }
      )
    ]
    ...
    #box(width: 3in)[
      #place(
        center,
        {
          line(start: (-60pt, 0pt), end: (-15pt, 0pt), stroke: 1pt + deep-slate)
          circle(radius: 4pt, fill: bright-cyan, stroke: 0.5pt + deep-slate)
          line(start: (15pt, 0pt), end: (60pt, 0pt), stroke: 1pt + deep-slate)
        }
      )
    ]
    ...
    // Bottom technical marks
    #box(width: 1.5in, height: 30pt)[
      #place(
        center,
        {
          // Measurement marks
          for i in range(7) {
            let x = -30pt + i * 10pt
            line(start: (x, -8pt), end: (x, if calc.rem(i, 2) == 0 { -12pt } else { -10pt }), stroke: 0.5pt + deep-slate)
          }
          // Center marker
          circle(radius: 2pt, fill: bright-cyan)
        }
      )
    ]
  ]
...
#page(
  ...
  #set text(size: 9pt, fill: rich-black)
  ...
  #align(center)[
    ...
    #box(width: 0.8in)[
      #place(
        center,
        {
          line(length: 0.8in, stroke: 0.5pt + deep-slate)
          place(dx: 0.4in, circle(radius: 2pt, fill: bright-cyan))
        }
      )
    ]
  ]
...
#page(
  ...
  #align(center)[
    ...
    #v(0.15in)
    #line(length: 2.5in, stroke: 1pt + deep-slate)
  ]

  #v(0.6in)

  #set par(first-line-indent: 0em, leading: 0.8em)
  #set text(size: 11pt, fill: rich-black)
